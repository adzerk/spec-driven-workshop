 ...
 *
 * If the methods are given `long` arguments, they will return a `long`.
 * If the methods are given `int` arguments, they will return an `int`.
 *
 * For comparison methods between an `a` or `b`, the return will be `a` or `b`.
 ...
 *
 * Some predicate methods within Java Math lib _should_ be branchless (via an Intrinsic hint),
 ...
 *
 * These branchless methods/functions can be useful for vectorized operations.
 **/
